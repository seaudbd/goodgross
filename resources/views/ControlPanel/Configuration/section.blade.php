...
@section('content')
                        ...
                        <div class="modal-body" style="padding-left: 80px; padding-right: 80px; padding-bottom: 50px;">
                            ...
                            <form id="add_form">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="category_type_id" type="text" class="form-control" id="category_type_id">
    ...
    <script language="JavaScript">
                    ...
                    if (result.total > 0) {
                        ...
                        if (result.total > '{{ $recordPerPage }}') {
                            ...
                            for (let i=1; i<=result.last_page; i++) {
                                let linkUrl = '{{ url('control/panel/configuration/section/get/records') }}/' + searchKey + '/{{ $recordPerPage }}?page=' + i;
                        ...
                        $.each(result.data, function (key, value) {
                            ...
                            $('#records').append($('<tr></tr>')
                                ...
                                .append('<td>' + value.section + '</div></td>')
        ...
        $(document).ready(function () {
            $('#search_key').val('');
            currentPageUrl = '{{ url('control/panel/configuration/section/get/records') }}/null/{{ $recordPerPage }}';
            gets(currentPageUrl);
        ...
        $(document).on('submit', '#search_form', function () {
            let searchKey = $('#search_key').val() === '' ? 'null' : $('#search_key').val();
            currentPageUrl = '{{ url('control/panel/configuration/section/get/records') }}/' + searchKey + '/{{ $recordPerPage }}';
            gets(currentPageUrl);
            return false;
        });
        ...
        $(document).on('click', '#bulk_apply', function () {
            ...
            $.ajax({
                method: 'post',
                url: '{{ url('control/panel/configuration/section/apply/bulk/operation') }}',
                data: data,
                contentType: false,
                processData: false,
                cache: false,
        ...
        $(document).on('submit', '#add_form', function () {
            ...
            $.ajax({
                method: 'post',
                url: '{{ url('control/panel/configuration/section/save/record') }}',
                data: data,
                processData: false,
                contentType: false,
                cache: false,
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                method: 'get',
                url: '{{ url('control/panel/configuration/section/get/record') }}',
                data: {
                    id: sectionId
                },
                cache: false,
                success: function (result) {
                    ...
                    $('#category_type_id').val(result.category_type_id);
        ...
        $(document).on('click', '.delete', function () {
            ...
            {{--$.ajax({--}}
                {{--method: 'post',--}}
                {{--url: '{{ url('control/panel/configuration/section/delete/record') }}',--}}
                {{--data: data,--}}
                {{--contentType: false,--}}
                {{--processData: false,--}}
                {{--cache: false,--}}
        ...
        $(document).on('change', '#jump_pagination', function () {
            ...
            if (isPositiveInteger(pageNumber) && pageNumber <= lastPageNumber) {
                let searchKey = $('#search_key').val() === '' ? 'null' : $('#search_key').val();
                currentPageUrl = '{{ url('control/panel/configuration/section/get/records') }}/' + searchKey + '/{{ $recordPerPage }}?page=' + pageNumber;
                gets(currentPageUrl);
            } else {
