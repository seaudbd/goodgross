...
@section('content')
    ...
    <script language="JavaScript">
                    ...
                    if (result.total > 0) {
                        ...
                        if (result.total > '{{ $recordPerPage }}') {
                            ...
                            for (let i=1; i<=result.last_page; i++) {
                                let linkUrl = '{{ url('control/panel/operation/posted/wholesale/items/get/records') }}/' + searchKey + '/{{ $recordPerPage }}?page=' + i;
        ...
        $(document).ready(function () {
            $('#search_key').val('');
            currentPageUrl = '{{ url('control/panel/operation/posted/wholesale/items/get/records') }}/null/{{ $recordPerPage }}';
            gets(currentPageUrl);
        ...
        $(document).on('submit', '#search_form', function () {
            let searchKey = $('#search_key').val() === '' ? 'null' : $('#search_key').val();
            currentPageUrl = '{{ url('control/panel/operation/posted/wholesale/items/get/records') }}/' + searchKey + '/{{ $recordPerPage }}';
            gets(currentPageUrl);
            return false;
        });
        ...
        $(document).on('click', '#bulk_apply', function () {
            ...
            if (status !== '' && ids.length > 0) {
                ...
                $.ajax({
                    method: 'post',
                    url: '{{ url('control/panel/operation/posted/wholesale/items/apply/bulk/operation') }}',
                    data: data,
                    contentType: false,
                    processData: false,
                    cache: false,
        ...
        $(document).on('change', '.posting_status', function () {
            ...
            $.ajax({
                method: 'post',
                url: '{{ url('control/panel/operation/posted/wholesale/items/change/status') }}',
                data: data,
                contentType: false,
                processData: false,
                cache: false,
        ...
        $(document).on('change', '#jump_pagination', function () {
            ...
            if (isPositiveInteger(pageNumber) && pageNumber <= lastPageNumber) {
                let searchKey = $('#search_key').val() === '' ? 'null' : $('#search_key').val();
                currentPageUrl = '{{ url('control/panel/operation/posted/wholesale/items/get/records') }}/' + searchKey + '/{{ $recordPerPage }}?page=' + pageNumber;
                gets(currentPageUrl);
            } else {
