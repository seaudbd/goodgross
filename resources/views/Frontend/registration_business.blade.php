...
<div class="container-fluid" id="registration_business_content">
    <div class="row">
            ...
            <nav class="navbar navbar-expand-sm navbar-light justify-content-center" style="padding: 0;">
                <ul class="navbar-nav">
                    <li class="nav-item" style="line-height: 1; padding-left: 15px; padding-right: 15px;">
                        <a class="nav-link" href="{{ url('register/personal') }}" style="color: #328C59; font-weight: 500;">Personal</a>
                    </li>
                    <li class="nav-item active primary_background_color_default" style="line-height: 1; padding-left: 15px; padding-right: 15px;">
                        <a class="nav-link" href="{{ url('register/business') }}" style="color: #ffffff; font-weight: 500;">Business</a>
                    </li>
                </ul>
            </nav>
            <div style="padding: 30px 50px 30px 50px;">
                <input type="hidden" id="password_show_hide_icon_status">
                <div id="registration_business_form_message" class="text-center text-danger pb-3"></div>
                <form id="registration_business_form">
                    <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                        <div class="col-12 col-sm-6 col-md-6 col-lg-6">
                            <input type="text" class="form-control" name="business_name" id="business_name" @if ($account !== null) value="{{ $account->business_name }}" @endif placeholder="Business Name">
                        </div>
                        <div class="col-12 col-sm-6 col-md-6 col-lg-6">
                            <input type="text" class="form-control" name="contact_number" id="contact_number" @if ($account !== null) value="{{ $account->contact_number }}" @endif placeholder="Business Phone Number">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-12 col-sm-6 col-md-6 col-lg-6">
                            <input type="text" class="form-control" name="email" id="email" @if ($account !== null) value="{{ $account->login_id }}" @endif placeholder="Business Email Address">
                        </div>
                        <div class="col-12 col-sm-6 col-md-6 col-lg-6">
                    ...
                    <div class="row mb-3">
                        <div class="col text-center">
                            <button type="submit" class="btn primary_btn_default btn-sm pr-3 pl-3" style="font-weight: 500;" id="registration_business_form_submit">Create Account</button>
                        </div>
                    </div>
                    <div class="row">
...
<script type="text/javascript">
    ...
    $(document).on('submit', '#registration_business_form', function(){
        $('#registration_business_form_submit').attr('disabled', 'disabled');
        $('#registration_business_form_message').empty();
        $('#registration_business_form').find('.is-invalid').removeClass('is-invalid');
        $('#registration_business_form').find('.error_message').remove();
        ...
        $.ajax({
            ...
            success: function (result) {
                console.log(result);
                $('#registration_business_form_submit').removeAttr('disabled');
                if (result.message === 'Account Created Successfully') {
                    location = '{{ url('load/email/verification') }}/' + result.account.id;
                } else {
                    $('#registration_business_form_message').text(result.message);
                }
            },
            error: function (xhr) {
                console.log(xhr);
